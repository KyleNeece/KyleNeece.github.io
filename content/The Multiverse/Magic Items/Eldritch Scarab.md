The Eldritch Scarab is a sentient parasite that attached itself to [[Akanos]]. The user has to be bonded to the Scarab 
###### Abilities 
The Eldritch Scarab gives a +2 to spell attack rolls and spell save dc
- As a bonus action you can change the appearance of the armor
- **Consuming Evolution**
	- The Scarab can consume different items (and sometimes parts of creatures, DM's discretion) in order to upgrade itself and grant the host new capabilities. 

**Consumed Items**
- ![[Arditi]]
	- The Blades can be retracted as a free action
- Potion of liquid lightning 
	- The Scarab can now shoot lightning energy blasts
		- It counts as a simple ranged weapon, with a normal range of 90 feet and a long range of 300 feet, and it deals 1d6 lightning damage on a hit. 
			- **So Attack Bonus** = Dexterity Modifier + Proficiency Bonus
			- **And Damage Bonus** = 1d6 Lightning plus dexterity modifier
				- **Note**: You can sneak attack with them 
- All-Purpose Tool 
	- As an action you can create a magical construct of a nonmagical item of your choice in an unoccupied space within 10 feet of you, the item has no money value. The created item disappears after a number of minutes equal to your proficiency bonus.
		- If you wish to create a more permanent object, the gp value of the item can't be more than 20 times your character level, and the item must be Medium or Smaller. Once you create such an object that lasts the same amount of time, you cannot do so again until you finish a long rest, unless you expend a spell slot of 2nd level or higher to do so again
- Winged Flight 
	- As an action you sprout wings that take the appearance of your choice. While the wings are sprouted you can use them to fly up to 4 hours, all at once or in several shorter flights, each using a minimum of 1 minute from the duration, if you are flying when the duration expires, you descend at a rate of 30 feet per round until you land. You can dismiss the wings as a action and it regains 2 hours of flying capability for every 12 hours not in use

